...
const searchOptions={
  ...
  strategy:['斬撃','貫通','打撃','憤怒','色欲','怠惰','暴食','憂鬱','傲慢','嫉妬','汎用','1章','2章','3.5章','4章','4.5章','5章','5.5-1章','5.5-2章','6章','6.5-1章','6.5-2章','7章','7.5-1章','7.5-2章','8章','8.5章','9章','9.5-1章','9.5-2章','オート対応','半オート','手動推奨','初心者向け','中級者向け','上級者向け','安定重視','高速周回','自由枠あり','人格固定','運要素あり','E.G.O依存','ギフト依存'],
  difficulty:['ノーマル','ハード'],
...
const storyStrategyTagOptions=['汎用','1章','2章','3.5章','4章','4.5章','5章','5.5-1章','5.5-2章','6章','6.5-1章','6.5-2章','7章','7.5-1章','7.5-2章','8章','8.5章','9章','9.5-1章','9.5-2章'];
...
function navigateToStep(target){
  if(postState.category==='mirror_railway'){
    const max=3*postState.railwayPartyCount+3;target=Math.max(1,Math.min(max,target));if(target>postState.step+1)target=postState.step+1;
    if(target===2&&postState.step===1&&!postState.railwayPartyPrompted){openRailwayPartyCountDialog();return false;}
    if(target<=postState.step){setStep(target);return true;}
    const physical=postState.step>=2&&postState.step<=1+3*postState.railwayPartyCount?2+((postState.step-2)%3):postState.step===2+3*postState.railwayPartyCount?6:7;
    if(physical<=3&&!validateRequiredStep(physical))return false;
...
function switchPartyForStep(next){
  if(postState.category==='mirror_railway'){
    const within=next>=2&&next<=1+3*postState.railwayPartyCount;if(!within){if(postState.railwayParties.length){postState.railwayParties[postState.activeRailwayPartyIndex]=captureActiveParty();postState.activeRailwayPartyIndex=0;loadParty(postState.railwayParties[0],'railway-0');}return;}
    const targetIndex=Math.floor((next-2)/3);if(targetIndex===postState.activeRailwayPartyIndex)return;
    postState.railwayParties[postState.activeRailwayPartyIndex]=captureActiveParty();postState.activeRailwayPartyIndex=targetIndex;
    loadParty(postState.railwayParties[targetIndex]||(postState.railwayParties[targetIndex]=blankPartyState()),`railway-${targetIndex}`);requestAnimationFrame(updatePartyKeywordSummary);return;
  }
  if(postState.category!=='projection_combat')return;const target=postState.secondaryPartyEnabled&&next>=5&&next<=7?'rear':'front';if(postState.activePartyPhase===target)return;if(postState.activePartyPhase==='rear')postState.rearParty=captureActiveParty();else postState.frontParty=frontParty=captureActiveParty();loadParty(target==='rear'?(postState.rearParty||(postState.rearParty=blankPartyState())):(postState.frontParty||frontParty||captureActiveParty()),target);requestAnimationFrame(updatePartyKeywordSummary);
}
const railwayPartyDialog=$('[data-railway-party-count-dialog]'),railwayPartyInput=$('[data-railway-party-count-input]'),railwayPartyError=$('[data-railway-party-count-error]'),railwayPartyLinks=$('[data-railway-party-step-links]');
function normalizePartyCount(value){const normalized=String(value||'').replace(/[０-９]/g,char=>String.fromCharCode(char.charCodeAt(0)-0xFEE0)).trim();const count=Number(normalized);return Number.isInteger(count)&&count>=1&&count<=5?count:0;}
function renderRailwayPartyStepLinks(){
  const railway=postState.category==='mirror_railway';if(railwayPartyLinks){railwayPartyLinks.hidden=!railway;railwayPartyLinks.innerHTML=railway?Array.from({length:postState.railwayPartyCount},(_,index)=>{const start=2+index*3;return `<div class="railway-party-step-group"><small>PARTY ${index+1}</small><button class="step-link" data-step-link="${start}" type="button"><span>${String(start).padStart(2,'0')}</span><div><strong>第${index+1}人格選択</strong><small>使用人格を選択</small></div></button><button class="step-link" data-step-link="${start+1}" type="button"><span>${String(start+1).padStart(2,'0')}</span><div><strong>第${index+1}編成順</strong><small>出撃順を選択</small></div></button><button class="step-link" data-step-link="${start+2}" type="button"><span>${String(start+2).padStart(2,'0')}</span><div><strong>第${index+1}使用E.G.O</strong><small>任意・E.G.Oを選択</small></div></button></div>`;}).join(''):'';railwayPartyLinks.querySelectorAll('[data-step-link]').forEach(button=>button.onclick=()=>navigateToStep(Number(button.dataset.stepLink)));}
  $$('[data-step-link="2"],[data-step-link="3"],[data-step-link="4"]').forEach(node=>{if(!node.closest('[data-railway-party-step-links]'))node.hidden=railway;});
  const detail=$('[data-detail-step-link]'),review=$('[data-review-step-link]');if(railway){if(detail){detail.dataset.stepLink=2+3*postState.railwayPartyCount;detail.querySelector('span').textContent=String(2+3*postState.railwayPartyCount).padStart(2,'0');}if(review){review.dataset.stepLink=3+3*postState.railwayPartyCount;review.querySelector('span').textContent=String(3+3*postState.railwayPartyCount).padStart(2,'0');}}
}
function openRailwayPartyCountDialog(){if(!railwayPartyDialog)return;railwayPartyInput.value=String(postState.railwayPartyCount||1);railwayPartyError.hidden=true;openDialog(railwayPartyDialog);requestAnimationFrame(()=>railwayPartyInput.focus());}
$('[data-confirm-railway-party-count]')?.addEventListener('click',()=>{const count=normalizePartyCount(railwayPartyInput.value);if(!count){railwayPartyError.textContent='1～5の数字を半角または全角で入力してください。';railwayPartyError.hidden=false;return;}postState.railwayPartyCount=count;postState.railwayPartyPrompted=true;postState.railwayParties=[captureActiveParty(),...Array.from({length:count-1},()=>blankPartyState())];postState.activeRailwayPartyIndex=0;renderRailwayPartyStepLinks();closeDialog(railwayPartyDialog);setStep(2);});
$('[data-cancel-railway-party-count]')?.addEventListener('click',()=>closeDialog(railwayPartyDialog));
...
postStepController=window.LimbusPostStepController.create({state:postState,stepInfo,identityData:sinnerIdentityData,onCloseEgo:closeEgoSelect,onCloseTheme:closeThemePackSelect,onRenderIdentities:renderIdentitySinnerRoster,onOpenIdentity:openIdentitySelect,onRenderFormation:renderFormationOrder,onRenderEgos:renderEgoSinners,onRenderThemes:renderThemeFloorCards,onRenderDetails:renderDetailTags,onRenderReview:updateReview,onIdentityFooterUpdate:updateIdentityFooterState,onEgoFooterUpdate:updateEgoConfirmState,onResetScroll:resetStageScroll,onBeforeStep:switchPartyForStep});
const postTagsController=window.LimbusPostTagsController.create({state:postState,strategyOptions:strategyTagOptions,getStrategyOptions:()=>{const extras=postState.category==='story'?storyStrategyTagOptions:postState.category==='luxcavation'?(postState.stage==='経験値'?['斬撃','貫通','打撃']:postState.stage==='紐'?['全属性','憤怒','色欲','怠惰','暴食','憂鬱','傲慢','嫉妬']:[]):[];const base=postState.category==='mirror_dungeon'?strategyTagOptions:strategyTagOptions.filter(tag=>tag!=='ギフト依存');return [...extras,...base.filter(tag=>!extras.includes(tag))];},affiliationOptions:affiliationTagOptions,automaticOptions:automaticKeywordOptions,manualOptions:manualKeywordOptions,strategyGrid:strategyTagGrid,affiliationGrid:affiliationTagGrid,automaticTags:automaticKeywordTags,ammoNote:ammoKeywordNote,strategyCount:strategyTagCount,affiliationCount:affiliationTagCount,getOrderedSinners:orderedSelectedSinners,getFormationPosition:formationPosition,isSolo:isSoloPost,showToast});
...
function applyCategoryEditorMode(){
  ...
  if(eventMode){if(stageMode&&postState.category!=='projection_combat')postState.difficulty=null;if(story)postState.stage=null;postState.themePacks.clear();if(postState.type&&!railwayTypes.has(postState.type))postState.type=null;}else{postState.stage=null;if(railwayTypes.has(postState.type))postState.type=null;}
  const lux=postState.category==='luxcavation';stepInfo[1][1]=story?'難易度と攻略タイプを選択':lux?'採光種類と攻略タイプを選択':eventMode?'ステージと攻略タイプを選択':'攻略タイプを選択';stepInfo[1][2]=story?'攻略する難易度と攻略方針を選んでください。':lux?'採光の種類と攻略方針を選んでください。':eventMode?'攻略するステージと攻略方針を選んでください。':'この攻略がどんなプレイヤー向けか選んでください。';
  const projection=postState.category==='projection_combat',projectionTotal=postState.secondaryPartyEnabled?9:6;const step1=$('[data-step-link="1"]'),step6=$('[data-detail-step-link]'),step7=$('[data-review-step-link]');if(step1){step1.querySelector('strong').textContent=story?'難易度・攻略タイプ':lux?'採光種類・攻略タイプ':eventMode?'ステージ・攻略タイプ':'攻略タイプ';step1.querySelector('small').textContent=projection?'ステージ・難易度・攻略方針':story?'難易度と攻略方針を選択':lux?'種類と攻略方針を選択':eventMode?'ステージと攻略方針を選択':'投稿の目的を選択';}$$('.projection-secondary-step').forEach(node=>node.hidden=!projection||!postState.secondaryPartyEnabled);$$('[data-step-link="2"] strong').forEach(node=>node.textContent=projection?'前半使用人格':'使用人格');$$('[data-step-link="3"] strong').forEach(node=>node.textContent=projection?'前半編成順':'編成順');$$('[data-step-link="4"] strong').forEach(node=>node.textContent=projection?'前半使用E.G.O':'使用E.G.O');if(step6){step6.dataset.stepLink=projection?(postState.secondaryPartyEnabled?'8':'5'):'6';step6.querySelector('span').textContent=projection?(postState.secondaryPartyEnabled?'08':'05'):eventMode?'05':'06';}if(step7){step7.dataset.stepLink=projection?(postState.secondaryPartyEnabled?'9':'6'):'7';step7.querySelector('span').textContent=projection?(postState.secondaryPartyEnabled?'09':'06'):eventMode?'06':'07';}
  ...
  renderRailwayPartyStepLinks();
  if(postState.category==='mirror_railway'&&guide)guide.textContent=`必須項目を満たしながら、${3*postState.railwayPartyCount+3}つの手順で攻略情報をまとめます。`;
}
