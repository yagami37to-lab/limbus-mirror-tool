...
function updateRailwayStageDisplay(){
  const eventMode=['mirror_railway','projection_combat','luxcavation'].includes(postState.category),label=postState.stage||(postState.category==='luxcavation'?'採光種類未選択':'ステージ未選択');
  ...
  if(eventMode){$$('[data-difficulty-badge]').forEach(node=>{node.textContent=label;node.classList.toggle('is-unset',!postState.stage);node.dataset.difficulty='';});const preview=$('[data-type-preview-difficulty]');if(preview){preview.textContent=postState.stage?`${postState.stage}攻略`:(postState.category==='luxcavation'?'採光種類を選択してください':'ステージを選択してください');preview.dataset.difficulty='';}}
}
function applyCategoryEditorMode(){
  ...
  if(difficulty)difficulty.hidden=eventMode;if(stage){stage.hidden=!eventMode;stage.classList.toggle('is-compact',eventMode&&postState.category!=='mirror_railway');}if(themeLink)themeLink.hidden=eventMode;if(themeReview)themeReview.hidden=eventMode;
  ...
  const step1=$('[data-step-link="1"]'),step6=$('[data-step-link="6"]'),step7=$('[data-step-link="7"]');if(step1){step1.querySelector('strong').textContent=lux?'採光種類・攻略タイプ':eventMode?'ステージ・攻略タイプ':'攻略タイプ';step1.querySelector('small').textContent=lux?'種類と攻略方針を選択':eventMode?'ステージと攻略方針を選択':'投稿の目的を選択';}if(step6)step6.querySelector('span').textContent=eventMode?'05':'06';if(step7)step7.querySelector('span').textContent=eventMode?'06':'07';
