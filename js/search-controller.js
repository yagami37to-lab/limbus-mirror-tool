...
// 攻略カテゴリ（検索・閲覧）
function selectBrowseCategory(categoryId,{scroll=true}={}){const category=categoryById(categoryId);if(!category.available){showToast(`${category.label}は実装予定です。`);return}selected.category.clear();selected.category.add(category.id);renderCategoryFilters(category.id);syncSearchControls();$$('.browse-category-tab').forEach(button=>button.classList.toggle('active',button.dataset.categoryId===category.id));const current=$('[data-current-browse-category]');if(current){current.textContent=category.label;current.dataset.categoryId=category.id}renderActiveFilters();runSearch(false);if(scroll)$('#community')?.scrollIntoView({behavior:'smooth'});}
