...
function createSearchController({
  ...
  const selected={category:new Set(),identity:new Set(),keyword:new Set(),type:new Set(),strategy:new Set(),difficulty:new Set(),projectionDifficulty:new Set(),affiliation:new Set()};
  let activeSelector='identity',activeTag='';
  const RETURN_PAGE_KEY='limbus-community-return-page';
  const RETURN_SORT_KEY='limbus-community-return-sort';
  const RETURN_DIRECTION_KEY='limbus-community-return-direction';
  ...
  const restoredSort=returningPost?(sessionStorage.getItem(RETURN_SORT_KEY)||'newest'):'newest';
  const restoredDirection=returningPost?(sessionStorage.getItem(RETURN_DIRECTION_KEY)||'desc'):'desc';
  if(sortSelect)sortSelect.value=restoredSort;if(resultSort)resultSort.value=restoredSort;if(sortDirection)sortDirection.value=restoredDirection;if(resultDirection)resultDirection.value=restoredDirection;
  const rememberBrowseState=()=>{sessionStorage.setItem(RETURN_PAGE_KEY,String(resultPage));sessionStorage.setItem(RETURN_SORT_KEY,resultSort?.value||sortSelect?.value||'newest');sessionStorage.setItem(RETURN_DIRECTION_KEY,resultDirection?.value||sortDirection?.value||'desc');};
  const dungeonTypes=['安定周回','速攻周回','高難易度向け','縛り・テーマ攻略','ネタ・ロマン','ソロ'];
  const eventTypes=['安定攻略','速攻攻略','低レア攻略'];
  const storyStrategy=['汎用','1章','2章','3.5章','4章','4.5章','5章','5.5-1章','5.5-2章','6章','6.5-1章','6.5-2章','7章','7.5-1章','7.5-2章','8章','8.5章','9章','9.5-1章','9.5-2章'];
  const dungeonStrategy=searchOptions.strategy.filter(tag=>!['斬撃','貫通','打撃','全属性','憤怒','色欲','怠惰','暴食','憂鬱','傲慢','嫉妬',...storyStrategy].includes(tag));
  const commonStrategy=dungeonStrategy.filter(tag=>tag!=='ギフト依存');
  const categoryFilterConfig={
    mirror_dungeon:{types:dungeonTypes,difficultyLabel:'難易度',difficulties:['ノーマル','ハード'],strategy:dungeonStrategy},
    ...
    story:{types:eventTypes,difficultyLabel:'難易度',difficulties:['ノーマル','ハード'],strategy:[...storyStrategy,...commonStrategy]}
  };
...
function addChipGroup(rootSelector,key,items,single=false,onChange=null){const root=$(rootSelector);if(!root)return;root.innerHTML='';items.forEach(item=>{const button=document.createElement('button');button.type='button';button.className='search-filter-chip';button.textContent=item;button.addEventListener('click',()=>{if(single){const was=selected[key].has(item);selected[key].clear();root.querySelectorAll('.search-filter-chip').forEach(x=>x.classList.remove('active'));if(!was){selected[key].add(item);button.classList.add('active')}}else{selected[key].has(item)?selected[key].delete(item):selected[key].add(item);button.classList.toggle('active')}onChange?.(item);renderActiveFilters();runSearch(false)});root.appendChild(button)})}
function renderCategoryFilters(categoryId,{reset=true}={}){const config=categoryFilterConfig[categoryId],projectionBlock=$('[data-search-projection-difficulty-block]'),isProjection=categoryId==='projection_combat';if(projectionBlock)projectionBlock.hidden=!isProjection;if(!config){if(difficultyLabel)difficultyLabel.textContent='難易度・ステージ';selected.projectionDifficulty.clear();addChipGroup('[data-search-types]','type',searchOptions.type,true);addChipGroup('[data-search-strategy-tags]','strategy',searchOptions.strategy);addChipGroup('[data-search-difficulties]','difficulty',['ノーマル','ハード','6号線','2号線','1号線','VS 中指 末兄 リカルド','VS 挿翅虎 レイホン','経験値','紐'],true);return}if(reset){selected.type.clear();selected.strategy.clear();selected.difficulty.clear();selected.projectionDifficulty.clear()}if(difficultyLabel)difficultyLabel.textContent=config.difficultyLabel;addChipGroup('[data-search-types]','type',config.types,true);const renderStrategy=()=>{let extras=[];const choice=[...selected.difficulty][0];if(categoryId==='luxcavation'&&choice==='経験値')extras=['斬撃','貫通','打撃'];if(categoryId==='luxcavation'&&choice==='紐')extras=['全属性','憤怒','色欲','怠惰','暴食','憂鬱','傲慢','嫉妬'];const options=[...extras,...config.strategy.filter(tag=>!extras.includes(tag))];const allowed=new Set(options);[...selected.strategy].forEach(tag=>{if(!allowed.has(tag))selected.strategy.delete(tag)});addChipGroup('[data-search-strategy-tags]','strategy',options)};renderStrategy();addChipGroup('[data-search-difficulties]','difficulty',config.difficulties,true,()=>{renderStrategy();syncSearchControls()});if(isProjection)addChipGroup('[data-search-projection-difficulties]','projectionDifficulty',['ノーマル','ハード'],true)}
function renderSearchCategories(){const root=$('[data-search-categories]');if(!root)return;root.innerHTML='';categoryDefinitions.forEach(category=>{const button=document.createElement('button');button.type='button';button.className='search-category-option';button.dataset.categoryId=category.id;button.disabled=!category.available;button.innerHTML=`<span class="search-category-icon">${categoryIconMarkup(category)}</span><span><strong>${category.label}</strong><small>${category.available?category.searchCopy:'※実装予定'}</small></span>`;button.addEventListener('click',()=>{if(!category.available)return;const was=selected.category.has(category.id);selected.category.clear();root.querySelectorAll('.search-category-option').forEach(x=>x.classList.remove('active'));if(!was){selected.category.add(category.id);button.classList.add('active')}renderCategoryFilters(was?'':category.id);$$('.browse-category-tab').forEach(tab=>tab.classList.toggle('active',was?tab.dataset.categoryId==='all':tab.dataset.categoryId===category.id));if(mobileBrowseCategory)mobileBrowseCategory.value=was?'':category.id;const current=$('[data-current-browse-category]');if(current){current.textContent=was?'すべて':category.label;current.dataset.categoryId=was?'all':category.id}renderActiveFilters();runSearch(false)});root.appendChild(button)})}
...
function syncSearchControls(){$$('.search-filter-chip').forEach(button=>{const key=button.closest('[data-search-keywords]')?'keyword':button.closest('[data-search-types]')?'type':button.closest('[data-search-strategy-tags]')?'strategy':button.closest('[data-search-projection-difficulties]')?'projectionDifficulty':button.closest('[data-search-difficulties]')?'difficulty':'affiliation';button.classList.toggle('active',selected[key].has(button.textContent))});$$('.search-category-option').forEach(button=>button.classList.toggle('active',selected.category.has(button.dataset.categoryId)));updateSelectionLabels()}
function clearSearch(){Object.values(selected).forEach(set=>set.clear());authorFilter='';renderCategoryFilters('',{reset:false});keywordInput.value='';$('[data-date-kind]').value='posted';$('[data-date-range]').value='all';sortSelect.value='newest';if(resultSort)resultSort.value='newest';if(sortDirection)sortDirection.value='desc';if(resultDirection)resultDirection.value='desc';syncSearchControls();renderActiveFilters();runSearch(false)}
...
function runSearch(scrollToResults=true,{resetPage=true}={}){
  ...
  const terms=[keywordInput.value.trim().toLowerCase(),activeTag.toLowerCase(),...['keyword','type','strategy','difficulty','projectionDifficulty','affiliation'].flatMap(key=>[...selected[key]].map(x=>x.toLowerCase()))].filter(Boolean);
...
$$('[data-open-selector]').forEach(b=>b.onclick=()=>openSelector(b.dataset.openSelector));$('[data-close-modal]').onclick=()=>closeDialog(selectorModal);modalClearSelection.onclick=()=>{selected.identity.clear();renderSelectionOptions(modalSearch.value)};modalApplySelection.onclick=()=>{updateSelectionLabels();closeDialog(selectorModal);runSearch(false)};modalSearch.oninput=e=>renderSelectionOptions(e.target.value);$('[data-clear-search]').onclick=clearSearch;keywordInput.oninput=()=>{renderActiveFilters();runSearch(false)};if(searchPanelToggle)searchPanelToggle.onclick=()=>setSearchPanelOpen(searchPanel?.hidden??true);$('[data-date-kind]').onchange=()=>{renderActiveFilters();runSearch(false)};$('[data-date-range]').onchange=()=>{renderActiveFilters();runSearch(false)};if(resultSort)resultSort.onchange=()=>{sortSelect.value=resultSort.value;rememberBrowseState();runSearch(false)};if(resultDirection)resultDirection.onchange=()=>{if(sortDirection)sortDirection.value=resultDirection.value;rememberBrowseState();runSearch(false)};advancedToggle.onclick=()=>{const open=advancedPanel.hidden;advancedPanel.hidden=!open;advancedToggle.setAttribute('aria-expanded',String(open));advancedToggle.innerHTML=open?'<span>−</span> 詳細条件を閉じる':'<span>＋</span> 詳細条件を表示'};
document.addEventListener('click',event=>{const detailLink=event.target.closest('a.post-detail-link[href*="post-detail.html"],a.post-detail-link[href*="request-detail.html"]');if(detailLink)rememberBrowseState();const b=event.target.closest('[data-tag]');if(!b)return;const on=b.classList.contains('active');$$('[data-tag]').forEach(x=>x.classList.remove('active'));activeTag=on?'':b.dataset.tag;if(!on)b.classList.add('active');renderActiveFilters();runSearch()});$$('[data-demo-detail]').forEach(b=>b.onclick=()=>showToast('詳細ページは次の段階で実装予定です。'));if(resultPagePrev)resultPagePrev.onclick=()=>{if(resultPage<=1)return;resultPage-=1;rememberBrowseState();runSearch(false,{resetPage:false});$('#community')?.scrollIntoView({behavior:'smooth',block:'start'})};if(resultPageNext)resultPageNext.onclick=()=>{resultPage+=1;rememberBrowseState();runSearch(false,{resetPage:false});$('#community')?.scrollIntoView({behavior:'smooth',block:'start'})};window.addEventListener('limbus-posts-loaded',()=>runSearch(false,{resetPage:false}));window.addEventListener('limbus-like-updated',()=>runSearch(false,{resetPage:false}));const scrollSearch=$('[data-scroll-search]');if(scrollSearch)scrollSearch.onclick=()=>setSearchPanelOpen(true,{scroll:true});document.querySelectorAll('a[href="#search"]').forEach(link=>link.addEventListener('click',event=>{event.preventDefault();setSearchPanelOpen(true,{scroll:true});}));

// 攻略カテゴリ（検索・閲覧）
function selectBrowseCategory(categoryId,{scroll=true}={}){if(!categoryId){selected.category.clear();renderCategoryFilters('',{reset:true});syncSearchControls();$$('.browse-category-tab').forEach(button=>button.classList.toggle('active',button.dataset.categoryId==='all'));if(mobileBrowseCategory)mobileBrowseCategory.value='';const current=$('[data-current-browse-category]');if(current){current.textContent='すべて';current.dataset.categoryId='all'}renderActiveFilters();runSearch(false);if(scroll)$('#community')?.scrollIntoView({behavior:'smooth'});return}const category=categoryById(categoryId);if(!category?.available){if(category)showToast(`${category.label}は実装予定です。`);return}selected.category.clear();selected.category.add(category.id);renderCategoryFilters(category.id);syncSearchControls();$$('.browse-category-tab').forEach(button=>button.classList.toggle('active',button.dataset.categoryId===category.id));if(mobileBrowseCategory)mobileBrowseCategory.value=category.id;const current=$('[data-current-browse-category]');if(current){current.textContent=category.label;current.dataset.categoryId=category.id}renderActiveFilters();runSearch(false);if(scroll)$('#community')?.scrollIntoView({behavior:'smooth'});}
function renderBrowseCategoryTabs(){const root=$('[data-browse-category-tabs]');if(!root)return;root.innerHTML='';const allButton=document.createElement('button');allButton.type='button';allButton.className='browse-category-tab browse-category-all';allButton.dataset.categoryId='all';allButton.innerHTML='<span class="browse-category-icon" aria-hidden="true">ALL</span><strong>全て</strong>';allButton.addEventListener('click',()=>selectBrowseCategory(''));root.appendChild(allButton);categoryDefinitions.forEach(category=>{const button=document.createElement('button');button.type='button';button.className='browse-category-tab';button.disabled=!category.available;button.dataset.categoryId=category.id;button.innerHTML=`<span class="browse-category-icon">${categoryIconMarkup(category)}</span><strong>${category.label}</strong>${category.available?'':'<small>※実装予定</small>'}`;button.addEventListener('click',()=>selectBrowseCategory(category.id));root.appendChild(button)})}
...
$$('.browse-category-tab').forEach(button=>button.classList.toggle('active',button.dataset.categoryId==='all'));if(mobileBrowseCategory)mobileBrowseCategory.value='';const initialCurrent=$('[data-current-browse-category]');if(initialCurrent){initialCurrent.textContent='すべて';initialCurrent.dataset.categoryId='all';}
