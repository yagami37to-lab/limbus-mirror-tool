...
async function hydrateSharedHeader(){
  try{
    ...
    imported.querySelectorAll('.header-nav a').forEach(link=>{const href=link.getAttribute('href')||'';if(href==='#featured'||href==='#search'){link.remove();return;}if(href.startsWith('#'))link.setAttribute('href',`index.html${href}`);if(href==='requests.html'){link.setAttribute('href','index.html');link.textContent='攻略投稿';link.classList.add('is-current');}});
