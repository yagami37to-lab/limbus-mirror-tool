(()=>{
'use strict';
function createPostPayloadController({state,identityData,getAlternativeNames,getOrderedSinners,getFormationPosition,getAutomaticKeywords}){
  ...
  function build(){
    ...
    return {title:value('[data-post-title]'),summary:value('[data-post-summary]'),category:state.category||'mirror_dungeon',difficulty:state.difficulty||null,strategy_type:state.type||null,content:{selectedIdentities,party,egos,themePacks:[...state.themePacks.entries()].map(([floor,name])=>({floor,name})),keywords:getAutomaticKeywords(),tags:[...state.strategyTags],affiliations:[...state.affiliationTags],description:value('[data-post-points]')}};
  }
  return {build};
}
