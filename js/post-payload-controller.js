...
function createPostPayloadController({state,identityData,getAlternativeNames,getOrderedSinners,getFormationPosition,getAutomaticKeywords,getSeason=()=>7}){
  ...
  function build(){
    ...
    return {title:value('[data-post-title]'),summary:value('[data-post-summary]'),category:state.category||'mirror_dungeon',difficulty:['mirror_railway','projection_combat'].includes(state.category)?null:(state.difficulty||null),strategy_type:state.type||null,content:{season:Number(getSeason())||7,stage:['mirror_railway','projection_combat'].includes(state.category)?(state.stage||null):null,selectedIdentities,party,egos,themePacks:['mirror_railway','projection_combat'].includes(state.category)?[]:[...state.themePacks.entries()].map(([floor,name])=>({floor,name})),keywords:getAutomaticKeywords(),tags:[...state.strategyTags],affiliations:[...state.affiliationTags],description:value('[data-post-points]')}};
  }
  return {build};
}
