(()=>{
'use strict';
function createPostCategoryController({state,categories,getCategory,iconMarkup,dialog,editorDialog,list,status,startButton,openDraftButton,openButtons,closeButton,categoryBadges,reviewCategory,isAuthenticated,openAuth,openDialog,closeDialog,openDrafts,resetEditor,getInitialSinnerId,setStep,onCategoryChanged}){
  let pendingCategory='';
  ...
  function startPost(){if(!pendingCategory)return;const selected=pendingCategory;resetEditor();state.category=selected;state.activeSinner=getInitialSinnerId();onCategoryChanged?.(selected);updateDisplays();setStep(1);closeDialog(dialog);requestAnimationFrame(()=>openDialog(editorDialog));}
