(() => {
  ...
  function wireSettings(){const select=qs('[data-theme-select]');if(select){select.value=document.documentElement.dataset.theme||'light';select.onchange=()=>applyTheme(select.value)}const anonymous=qs('[data-anonymous-posting]');if(anonymous){const key=`limbus-anonymous-posting:${user.id}`;const sync=()=>{const enabled=localStorage.getItem(key)==='true';anonymous.setAttribute('aria-checked',String(enabled));anonymous.querySelector('b').textContent=enabled?'ON':'OFF'};sync();anonymous.onclick=()=>{const enabled=localStorage.getItem(key)!=='true';localStorage.setItem(key,String(enabled));sync();notice(enabled?'匿名投稿を有効にしました。':'匿名投稿を解除しました。','success')}}qs('[data-clear-bookmarks]')?.addEventListener('click',async()=>{if(!confirm('クラウド上のブックマークをすべて解除しますか？'))return;await client.from('bookmarks').delete().eq('user_id',user.id);Object.keys(localStorage).filter(k=>k.startsWith('limbus-bookmark:')).forEach(k=>localStorage.removeItem(k));notice('ブックマークをすべて解除しました。','success');fillIdentity({bookmarkCount:0,postCount:cloudPosts.length,likeCount:cloudPosts.reduce((sum,p)=>sum+Number(p.likes||0),0)})});qs('[data-account-logout-page]')?.addEventListener('click',async()=>{await client.auth.signOut();location.href='index.html'});}
