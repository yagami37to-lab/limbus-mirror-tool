(() => {
  ...
  async function renderBookmarks(){
    ...
    const cards=await Promise.all(merged.map(async p=>{
      const profile=await api.getProfile(p.author_id);const keywords=Array.isArray(p.keywords)?p.keywords:[];const tags=Array.isArray(p.tags)?p.tags:[];const affiliations=Array.isArray(p.affiliations)?p.affiliations:[];const party=Array.isArray(p.party)?p.party:[];const modeLabel=['mirror_railway','projection_combat','luxcavation'].includes(p.category)?(p.stage||(p.category==='luxcavation'?'採光種類未設定':'ステージ未設定')):(p.difficulty==='HARD'?'ハード':p.difficulty==='NORMAL'?'ノーマル':p.difficulty||'未設定');
