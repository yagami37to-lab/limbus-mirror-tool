(async()=>{
  ...
  try{
    ...
    for(const p of posts){
      ...
      const modeLabel=['mirror_railway','projection_combat','luxcavation'].includes(p.category)?(c.stage||(p.category==='luxcavation'?'採光種類未設定':'ステージ未設定')):(p.difficulty==='HARD'?'ハード':p.difficulty==='NORMAL'?'ノーマル':p.difficulty||'未設定');
