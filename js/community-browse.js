(async()=>{
  ...
  try{
    const posts=await api.getPublishedPosts({limit:50});
    grid.innerHTML='';
    for(const p of posts){
      ...
      await sync();
      save.onclick=async e=>{e.preventDefault();if(currentUser?.id===p.author_id)return;const user=await api.sessionUser();if(!user){window.LimbusAuth?.open();return}try{const on=await api.isBookmarked(p.id);await api.setBookmark(p.id,!on);await sync();}catch(error){console.error(error);window.alert(error.message==='own-post-bookmark'?'自分の投稿はブックマークできません。':'ブックマークを更新できませんでした。');}};
    }
