(async()=>{
  ...
  try{
    ...
    for(const p of posts){
      ...
      const projectionStage=String(c.stage||'').includes('リカルド')?'VS リカルド':String(c.stage||'').includes('レイホン')?'VS レイホン':'ステージ未設定';
      const modeLabel=p.category==='projection_combat'?projectionStage:['mirror_railway','luxcavation'].includes(p.category)?(c.stage||(p.category==='luxcavation'?'採光種類未設定':'ステージ未設定')):(p.difficulty==='HARD'?'ハード':p.difficulty==='NORMAL'?'ノーマル':p.difficulty||'未設定');
      const normalizedTitle=String(p.title||'').replace(/[＋+]/g,'+').replace(/\s+/g,'');
      const achievementTurns=Number(c.achievementTurns)||(p.category==='projection_combat'&&normalizedTitle==='中指+薬指ファウスト'?12:0);
      ...
      const categoryBadge=card.querySelector('.browse-category-badge');
      if(categoryBadge)categoryBadge.dataset.categoryId=p.category||'mirror_dungeon';
      if(p.category==='projection_combat')card.querySelector('.browse-difficulty')?.insertAdjacentHTML('afterend',`<span class="difficulty-badge projection-difficulty-badge" data-difficulty="${esc(p.difficulty||'NORMAL')}">${projectionDifficulty}</span>`);
      grid.appendChild(card);
      window.LimbusOwnedIdentities?.applyCard?.(card,party,egos);
      if(p.category==='projection_combat'&&achievementTurns){const rate=card.querySelector('.post-owned-rate');rate?.insertAdjacentHTML('beforebegin',`<span class="projection-turn-badge projection-turn-by-rate">${achievementTurns}ターン達成</span>`);}
