...
function createPostValidationController({state,requiredIdentityCount}){
  ...
  function check(step){
    if(step===1){if(!value('[data-post-title]'))return {valid:false,message:'攻略タイトルを設定していません',field:'title',popup:true};if(state.category==='mirror_railway'){if(!state.stage&&!state.type)return {valid:false,message:'ステージと攻略タイプを選択してください。',field:'stage'};if(!state.stage)return {valid:false,message:'6号線・2号線・1号線からステージを選択してください。',field:'stage'};}else{if(!state.difficulty&&!state.type)return {valid:false,message:'難易度と攻略タイプを選択してください。',field:'difficulty'};if(!state.difficulty)return {valid:false,message:'ノーマルまたはハードを選択してください。',field:'difficulty'};}if(!state.type)return {valid:false,message:'攻略タイプを選択してください。',field:'type'};}
