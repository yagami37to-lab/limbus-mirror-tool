(()=>{
'use strict';
function createPostAdvanceController({state,identityCount,closeEgoSelect,closeThemePackSelect,publish,confirmFillEmpty,fillEmpty,afterFillEmpty,validateStep,validateDetails,setStep,confirmSecondary,onSecondaryChanged,requestRailwayParties,isRequest=()=>false}){
  async function advance(){
    if(state.category==='mirror_railway'&&state.step===1&&!state.railwayPartyPrompted&&!isRequest()){if(!validateStep(1))return false;requestRailwayParties?.();return false;}
    if(state.category==='mirror_railway'){const count=Math.max(1,Number(state.railwayPartyCount)||1),max=3*count+3,partyStep=state.step>=2&&state.step<=1+3*count,physical=partyStep?2+((state.step-2)%3):state.step===2+3*count?6:state.step===max?7:state.step;if(physical===4&&state.activeEgoSinner){closeEgoSelect();return false;}if(state.step===max)return publish();if(physical===2&&state.identities.size<identityCount&&!isRequest()){const missingCount=identityCount-state.identities.size;if(!confirmFillEmpty(missingCount))return false;fillEmpty();afterFillEmpty(missingCount);}if((physical===2||physical===3||physical===6)&&!validateStep(physical))return false;setStep(state.step+1);return true;}
    ...
    if(state.step===2&&state.identities.size<identityCount&&!isRequest()){
    ...
    if(onProjectionDetails&&!isRequest()){if(!validateDetails())return false;}else if(!(projection&&state.step>=5&&state.step<=7)&&!validateStep(validationStep))return false;
