...
function createPostStepController({state,stepInfo,identityData,onCloseEgo,onCloseTheme,onRenderIdentities,onOpenIdentity,onRenderFormation,onRenderEgos,onRenderThemes,onRenderDetails,onRenderReview,onEnterIdentity,onIdentityFooterUpdate,onEgoFooterUpdate,onResetScroll}){
  ...
  function set(step){
    ...
    if(state.step===2){onRenderIdentities();if(state.activeSinner)onOpenIdentity(state.activeSinner,{scroll:false});if(previous!==2)onEnterIdentity?.();}if(state.step===3)onRenderFormation();if(state.step===4)onRenderEgos();if(state.step===5)onRenderThemes();if(state.step===6)onRenderDetails();if(state.step===7)onRenderReview();
