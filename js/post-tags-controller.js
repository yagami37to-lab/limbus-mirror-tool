(()=>{
'use strict';
function createPostTagsController({state,strategyOptions,getStrategyOptions,affiliationOptions,automaticOptions,manualOptions,strategyGrid,affiliationGrid,automaticTags,ammoNote,strategyCount,affiliationCount,getOrderedSinners,getFormationPosition,isSolo,showToast}){
  function counts(){
  ...
  function automaticKeywords(){if(['mirror_railway','projection_combat','luxcavation','story'].includes(state.category))return [...state.manualKeywords];const result=counts();const tags=automaticOptions.filter(keyword=>result.get(keyword)>=5);if(state.ammoKeywordSelected&&result.get('弾丸')>=1)tags.push('弾丸');if(result.get('ソロ')===1)tags.push('ソロ');return tags;}
  ...
  function renderButtons(grid,options,selection,max){if(!grid)return;grid.innerHTML='';options.forEach(tag=>{const button=document.createElement('button');button.type='button';button.className='selectable-tag'+(selection.has(tag)?' selected':'');button.dataset.tag=tag;button.textContent=tag;button.addEventListener('click',()=>toggleTag(tag,selection,max));grid.appendChild(button);});}
  function render(){
    renderButtons(strategyGrid,getStrategyOptions?.()||strategyOptions,state.strategyTags,5);renderButtons(affiliationGrid,affiliationOptions,state.affiliationTags,3);if(strategyCount)strategyCount.textContent=state.strategyTags.size;if(affiliationCount)affiliationCount.textContent=state.affiliationTags.size;
    if(['mirror_railway','projection_combat','luxcavation','story'].includes(state.category)){renderButtons(automaticTags,manualOptions,state.manualKeywords,5);if(ammoNote)ammoNote.hidden=true;return;}
