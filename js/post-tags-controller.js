(()=>{
'use strict';
function createPostTagsController({state,strategyOptions,affiliationOptions,automaticOptions,strategyGrid,affiliationGrid,automaticTags,ammoNote,strategyCount,affiliationCount,getOrderedSinners,getFormationPosition,isSolo,showToast}){
  function counts(){
  ...
  function automaticKeywords(){const result=counts();const tags=automaticOptions.filter(keyword=>result.get(keyword)>=5);if(state.ammoKeywordSelected&&result.get('弾丸')>=1)tags.push('弾丸');if(result.get('ソロ')===1)tags.push('ソロ');return tags;}
