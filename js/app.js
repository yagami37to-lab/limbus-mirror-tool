...
const selectorModal=$('[data-selector-modal]'),categoryPicker=$('[data-category-picker]'),postModal=$('[data-post-modal]'),selectionGrid=$('[data-selection-grid]');
...
const resultCount=$('[data-result-count]'),emptyState=$('[data-empty-state]'),toast=$('[data-toast]');
...
[selectorModal,categoryPicker,postModal].filter(Boolean).forEach(dialog=>{
...
function renderActiveFilters(){const items=getFilterItems();if(activeTag)items.push({key:'popular',value:activeTag,label:`人気検索：${activeTag}`});renderFilterTarget(activeFilters,items,'現在、検索条件は指定されていません。');renderFilterTarget(resultActiveFilters,items,'すべての攻略を表示しています。')}
...
function runSearch(scrollToResults=true){const identityTerms=[...selected.identity].map(x=>x.split('｜')[0].toLowerCase());const terms=[keywordInput.value.trim().toLowerCase(),activeTag.toLowerCase(),...['keyword','type','strategy','difficulty','affiliation'].flatMap(key=>[...selected[key]].map(x=>x.toLowerCase())),...identityTerms].filter(Boolean);const selectedCategory=[...selected.category][0]||'';const cards=postGrid?[...postGrid.querySelectorAll(':scope > .post-card')]:[];cards.forEach(c=>{const h=(c.dataset.title+' '+c.dataset.tags+' '+(c.dataset.identities||'')+' '+c.textContent).toLowerCase();const categoryMatches=!selectedCategory||c.dataset.category===selectedCategory;c.hidden=!categoryMatches||!terms.every(t=>h.includes(t))});let visible=cards.filter(c=>!c.hidden);const sortValue=resultSort?.value||sortSelect.value;if(sortValue==='views')visible.sort((a,b)=>+b.dataset.views-+a.dataset.views);if(sortValue==='popular'||sortValue==='recommended')visible.sort((a,b)=>+b.dataset.popular-+a.dataset.popular);if(sortValue==='newest'||sortValue==='updated')visible.reverse();visible.forEach(c=>postGrid.appendChild(c));resultCount.textContent=visible.length;emptyState.hidden=visible.length!==0;renderActiveFilters();if(scrollToResults){showToast(`${visible.length}件の攻略を表示しました。`);$('#community').scrollIntoView({behavior:'smooth'})}}
$$('[data-open-selector]').forEach(b=>b.onclick=()=>openSelector(b.dataset.openSelector));$('[data-close-modal]').onclick=()=>closeDialog(selectorModal);modalClearSelection.onclick=()=>{selected.identity.clear();renderSelectionOptions(modalSearch.value)};modalApplySelection.onclick=()=>{updateSelectionLabels();closeDialog(selectorModal)};modalSearch.oninput=e=>renderSelectionOptions(e.target.value);$('[data-search]').onclick=runSearch;$('[data-clear-search]').onclick=clearSearch;keywordInput.oninput=renderActiveFilters;keywordInput.onkeydown=e=>{if(e.key==='Enter')runSearch()};$('[data-date-kind]').onchange=renderActiveFilters;$('[data-date-range]').onchange=renderActiveFilters;if(resultSort)resultSort.onchange=()=>runSearch(false);advancedToggle.onclick=()=>{const open=advancedPanel.hidden;advancedPanel.hidden=!open;advancedToggle.setAttribute('aria-expanded',String(open));advancedToggle.innerHTML=open?'<span>−</span> 詳細条件を閉じる':'<span>＋</span> 詳細条件を表示'};
document.addEventListener('click',event=>{const b=event.target.closest('[data-tag]');if(!b)return;const on=b.classList.contains('active');$$('[data-tag]').forEach(x=>x.classList.remove('active'));activeTag=on?'':b.dataset.tag;if(!on)b.classList.add('active');renderActiveFilters();runSearch()});$$('[data-demo-detail]').forEach(b=>b.onclick=()=>showToast('詳細ページは次の段階で実装予定です。'));$('[data-load-more]').onclick=()=>showToast('追加投稿の読み込みはデータ接続時に実装します。');$('[data-scroll-search]').onclick=()=>$('#search').scrollIntoView({behavior:'smooth'});
...
function applyIdentityCardImage(element,imageUrl){
  ...
  element.style.backgroundImage=`linear-gradient(180deg,rgba(0,0,0,.02) 0%,rgba(0,0,0,.10) 28%,rgba(0,0,0,.84) 64%,rgba(0,0,0,.98) 100%),url("${safeUrl}")`;
} 
...
function renderIdentityAlternativeControls(){
  ...
  const sinner=sinnerIdentityData.find(x=>x.id===postState.activeSinner);const primary=postState.identities.get(postState.activeSinner);const names=alternativeNamesFor(postState.activeSinner);
  if(!sinner||!primary||primary.isFreeSlot){root.hidden=true;return;}
  root.hidden=false;root.innerHTML=`<div><strong>代用人格（任意）</strong><small>${names.length?names.map(reviewEscape).join('、'):'未設定'}</small></div><button type="button" data-toggle-alternatives>${postState.alternativeSelectionMode?'代用人格の選択を完了':'＋代用人格を追加（任意）'}</button>`;
...
function renderIdentityOptions(){
  ...
  if(showFreeSlot){
    ...
    free.addEventListener('click',()=>{
      ...
      postState.identityAlternatives.delete(sinner.id);
...
selectorModal.addEventListener('click',e=>{if(e.target===selectorModal)closeDialog(selectorModal)});
postState.activeSinner=sinnerIdentityData[0]?.id||null;updatePostCategoryDisplays();renderActiveFilters();renderIdentitySinnerRoster();updatePartyKeywordSummary();setStep(1);
