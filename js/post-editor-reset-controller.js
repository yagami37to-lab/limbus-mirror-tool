...
function createPostEditorResetController({state,identityData,postModal,onClearDraft,onRefresh}){
  function reset(){
    ...
    state.step=1;state.category='mirror_dungeon';state.type=null;state.difficulty=null;state.identities=new Map();state.identityAlternatives=new Map();state.identityOrder=[];state.egos=new Map();state.freeSlotEgoEnabled=new Set();state.themePacks=new Map();state.activeThemePackFloor=null;state.strategyTags=new Set();state.affiliationTags=new Set();state.ammoKeywordSelected=false;state.activeSinner=identityData[0]?.id||null;state.activeEgoSinner=null;state.alternativeSelectionMode=false;
    ...
    document.querySelectorAll('[data-post-type],[data-post-difficulty]').forEach(node=>node.classList.remove('active'));document.querySelectorAll('.validation-error').forEach(node=>node.classList.remove('validation-error'));
    onRefresh();
  }
  return {reset};
}
